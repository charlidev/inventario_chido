<?php
    ...
    $id = $_POST['id'];
    $nombre = $_POST['nombreEDepen'];
    $status = $_POST['estatusEDepen'];
    ...
    if(empty($nombre) || empty($status)){
        ...
        $tsql = "UPDATE tblDependencia SET Nombre = ?, Estatus = ? WHERE idDependencia = ?";
        ...
        if($stmt){ //validamos si se encontro el registro
            $datos['status'] = 1;
            $datos['msg'] = "!Dependencia editada satisfactoriamente!";
        }
